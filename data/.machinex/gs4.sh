...
case "$1" in
	CPUFrequencyList)
		for CPUFREQ in `$BB cat /sys/devices/system/cpu/cpu0/cpufreq/machinex_available_frequencies`; do
		LABEL=$((CPUFREQ / 1000));
	...
	Hotplug)
		...
		if [ "$(cat /sys/devices/platform/msm_sleeper/enabled)" -eq "1" ]; then
			MSM_SLEEPER=Active;
		else
			MSM_SLEEPER=Inactive;
		fi;
		...
		echo "Default HotPlug: $DEFAULT_HOTPLUG@nLazyplug: $LAZYPLUG@nIntelli_Hotplug: $INTELLI_HOTPLUG@nBricked_Hotplug: $BRICKED_HOTPLUG@nMSM_Sleeper: $MSM_SLEEPER@nAlucard_Hotplug: $ALUCARD_HOTPLUG"
	;;
	DirGPUMaxFrequency)
